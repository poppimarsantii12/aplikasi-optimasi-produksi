...
# Menggunakan math.floor untuk memastikan batas atas integer
x_vals = np.arange(1, math.floor(min(x_intercept1, x_intercept2)) + 1)
y_vals = np.arange(1, math.floor(min(y_intercept1, y_intercept2)) + 1)
...
if valid_solutions:
    # Filter solusi agar x dan y lebih besar dari 0
    filtered_solutions = [s for s in valid_solutions if s['x'] > 0 and s['y'] > 0]
    if filtered_solutions:
        best = max(filtered_solutions, key=lambda item: item['profit'])
...
if optimal_point != (0, 0):
    st.success(f"✅ Solusi Optimal: {optimal_point[0]} Meja dan {optimal_point[1]} Kursi")
...
# Pastikan rentang x_vals_plot cukup besar untuk menangani intercept
max_x_plot = max(x_intercept1, x_intercept2, optimal_point[0] if optimal_point else 0) * 1.2
x_vals_plot = np.linspace(0, max_x_plot, 400)
...
# Plot titik potong batas jika valid
if intersect_point[0] > 0 and intersect_point[1] > 0 and \
   intersect_point[0] <= max_x_plot and intersect_point[1] <= max(y_intercept1, y_intercept2, optimal_point[1] if optimal_point else 0) * 1.2:
...
ax.set_xlim(0, max_x_plot)
ax.set_ylim(0, max(y_intercept1, y_intercept2, optimal_point[1] if optimal_point else 0) * 1.2)
ax.legend()
ax.grid(True)
st.pyplot(fig)
...
# --- TABEL TITIK VALID ---
if best_solution:
    st.markdown("### 📋 Titik Produksi Optimal")
...
with st.expander("📈 Lihat Proses Perhitungan"):
    if best_solution:
        ...
        st.subheader("Fungsi Tujuan dengan Angka:")
